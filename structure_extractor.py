...
class Overlap():
    ...
    def process(self, x: torch.Tensor, y: torch.Tensor):
        single_channel_x = x[0,0,:,:]
        single_channel_y = y[0,0,:,:]
        pic_x = torch.ones_like(single_channel_x) # binary 
        pic_
...
# def try_cryptograph():
#     lenna = cv2.imread("/home/zhangyushan/kyoma/lenna_single_channel.png") #(512,512)
#     img_channels = 3
#     out_channels = img_channels * 2
#     img_size = lenna.shape[0]

#     lenna = transforms.Compose([transforms.ToTensor(),])(lenna)
#     batch_images = torch.unsqueeze(lenna, 0)
#     gen = Gray_Generator(img_channels=img_channels, out_channels=out_channels)
#     res = gen(batch_images)
#     overlap = Overlap("cpu")
#     res = overlap.process(res)
#     res*=255
#     print(res.shape)
#     res = res[0].detach().numpy()
#     res = res.transpose(1,2,0)
#     cv2.imwrite("/home/zhangyushan/kyoma/lenna_overlap.png", res)
...
if __name__ == "__main__":
    ...
    # print(result.shape)
    #  ==== following kyoma added
    lenna = read_img("/home/zhangyushan/kyoma/only_target/2_256.jpg")
    batch_images = torch.unsqueeze(lenna, 0)
    super_pixel = SuperPixel(mode='simple')
    dog = DoG()
    guided_filter = GuidedFilter()
    with torch.no_grad():
        pixel = np.ones((512, 512, 3))
        base_color = super_pixel.process(batch_images)[0]
        base_color = base_color.numpy()
        base_color = base_color.transpose(1, 2, 0) # (512, 512, 3)
        lines = dog.process(batch_images)[0]
        lines = lines[0]
        lines = lines.numpy()
        lines = lines.transpose(1,2,0)
        n1 = base_color.shape[0]
        m1 = base_color.shape[1]

        pixel = np.ones((m1, n1, 3))
        scale = 0.5
        for i in range(n1):
            for j in range(m1): 
                if lines[i,j,0] < 0.1:
                    # r = g = b = 255 * lines[i,j,0]
                    r = 255 * base_color[i, j, 0] * scale
                    g = 255 * base_color[i, j, 1] * scale
                    b = 255 * base_color[i, j, 2] * scale
                else:
                    r = 255 * base_color[i, j, 0]
                    g = 255 * base_color[i, j, 1]
                    b = 255 * base_color[i, j, 2]
                pixel[i,j] = int(r), int(g), int(b)
        # pixel = pixel / 255.0
        # pixel = pixel.transpose(2,0,1)
        # smooth = torch.from_numpy(pixel).float()
        # smooth = torch.unsqueeze(smooth, 0)
        # smooth = guided_filter.process(smooth, batch_images, r=5)[0]
        # smooth *= 255
        # smooth = smooth.numpy()
        # smooth = smooth.transpose(1,2,0)
        # res = np.ones((m1, n1, 3))
        # for i in range(n1):
        #     for j in range(m1): 
        #         r = smooth[i, j, 0]
        #         g = smooth[i, j, 1]
        #         b = smooth[i, j, 2]
        #         res[i, j] = int(b), int(g), int(r)
        cv2.imwrite("/home/zhangyushan/kyoma/bird.png", pixel)
