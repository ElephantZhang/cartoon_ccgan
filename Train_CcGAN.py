...
def train_CcGAN(kernel_sigma, kappa, photos, train_images, train_labels, gen, disc_surface, disc_texture, net_y2h_surface, net_y2h_texture, VGG, save_images_folder, save_models_folder = None, clip_label=False):
    ...
    for niter in range(resume_niters, niters):
        ...
        batch_target_texture_labels_in_dataset = np.random.choice(unique_train_labels, size=batch_size_max, replace=True)
        ...
        batch_target_texture_labels_with_epsilon = batch_target_texture_labels_in_dataset + batch_epsilons
        if clip_label:
            batch_target_surface_labels_with_epsilon = np.clip(batch_target_surface_labels_with_epsilon, 0.0, 1.0)
            batch_target_texture_labels_with_epsilon = np.clip(batch_target_texture_labels_with_epsilon, 0.0, 1.0)

        batch_target_surface_labels = batch_target_surface_labels_with_epsilon[0:batch_size_disc]
        batch_target_texture_labels = batch_target_texture_labels_with_epsilon[0:batch_size_disc]
        ...
        batch_real_texture_indx = np.zeros(batch_size_disc, dtype=int)
        batch_fake_surface_labels = np.zeros(batch_size_disc)
        batch_fake_texture_labels = np.zeros(batch_size_disc)
        ...
        for j in range(batch_size_disc):
            ...
            if threshold_type == "hard":
                ...
                indx_real_texture_in_vicinity = np.where((train_labels-batch_target_texture_labels[j])**2 <= -np.log(nonzero_soft_weight_threshold)/kappa)[0]
            ...
            while len(indx_real_surface_in_vicinity)<1:
                ...
                if threshold_type == "hard":
                    ...
                    # reverse the weight function for SVDL
                    indx_real_surface_in_vicinity = np.where((train_labels-batch_target_surface_labels[j])**2 <= -np.log(nonzero_soft_weight_threshold)/kappa)[0]
            while len(indx_real_texture_in_vicinity)<1:
                batch_epsilons_j = np.random.normal(0, kernel_sigma, 1)
                batch_target_texture_labels[j] = batch_target_texture_labels_in_dataset[j] + batch_epsilons_j
                if clip_label:
                    batch_target_texture_labels = np.clip(batch_target_texture_labels, 0.0, 1.0)
                ## index for real images
                if threshold_type == "hard":
                    # indx_real_in_vicinity = np.where(np.abs(train_labels-batch_target_labels[j])<= kappa)[0]
                    assert False
            ...
            assert len(indx_real_texture_in_vicinity)>=1

            batch_real_surface_indx[j] = np.random.choice(indx_real_surface_in_vicinity, size=1)[0]
            batch_real_texture_indx[j] = np.random.choice(indx_real_texture_in_vicinity, size=1)[0]
            ...
            if threshold_type == "hard":
                ...
                lb_texture = batch_target_texture_labels[j] - np.sqrt(-np.log(nonzero_soft_weight_threshold)/kappa)
                ub_surface = batch_target_surface_labels[j] + np.sqrt(-np.log(nonzero_soft_weight_threshold)/kappa)
                ub_texture = batch_target_texture_labels[j] + np.sqrt(-np.log(nonzero_soft_weight_threshold)/kappa)
        ...
        if (niter+1) % 25 == 0:
            ...
            sw.add_scalar("D surface loss", d_surface_loss.item(), niter)
            sw.add_scalar("D texture loss", d_texture_loss.item(), niter)
    ...
    #end for niter
    return gen, disc_surface, disc_texture
