...
class ResidualBlock(nn.Module): # B, 128, 64, 64
    ...
    def forward(self, x, y_surface):
        #Elementwise Sum (ES)
        x_ = self.cond_batchnorm(x, y_surface)
        x_ = self.conv0(x_)
        x_ = self.relu(x_)
        x_ = self.cond_batchnorm(x_, y_surface)
        x_ = self.conv1(x_)
...
class Up1(nn.Module):
    ...
    def forward(self, x, y_surface):
        x = self.conv(x)
        x = self.cond_batchnorm(x, y_surface)
        x = self.relu(x)
        return x
...
class Up2(nn.Module):
    ...
    def forward(self, x, y_surface):
        x = self.conv0(x)
        x = self.cond_batchnorm0(x, y_surface)
        x = self.relu0(x)
        x = self.conv1(x)
        x = self.cond_batchnorm1(x, y_surface)
        x = self.relu1(x)
        return x
...
class Generator(nn.Module):
    ...
    def forward(self, x, y_surface): # B, C, 256, 256
        # y are  embed label
